...
Partial Class UserControlsTaskSearch
    ...
    Private Sub AddControlsToTable(ByVal control As Object, ByVal lblControl As Label, ByVal colSpan As Integer, Optional ByVal newLineAfterLabel As Boolean = True)
        With Me._tblTaskSearch
            ...
            If lblControl.Text.Length > 0 Then
                ...
                End If
            End If
            currentCell.Controls.Add(control)
            ...
            '    currentCellIndex = .Rows(currentRowIndex).Cells.Add(currentCell)
            '    currentRow.CssClass = "Border"
            '    availableCells = 4 - colSpan
            'Else
            '    'currentCellIndex = .Rows(currentRowIndex).Cells.Count - 1
            '    currentRow = .Rows(currentRowIndex)
            '    currentCellIndex = .Rows(currentRowIndex).Cells.Add(currentCell)
            '    currentCell = .Rows(currentRowIndex).Cells.Item(currentCellIndex)
            '    currentCell.ColumnSpan = colSpan
            '    availableCells = GetAvailableCells(currentRow, maxColumns) '- colSpan ' 4 - colSpan - currentCellIndex
            '    If availableCells > 0 Then
            '        currentCellIndex = currentRow.Cells.Add(currentCell)
            '        currentRow.CssClass = "Border"
            '    Else
            '        currentRowIndex = .Rows.Add(currentRow)
            '        'Add new cell
    ...
    Private Sub PopulateReportTitles(ByVal application As String)
        ...
        If Not Page.IsPostBack Then
            ...
            If reportTitleList IsNot Nothing AndAlso reportTitleList.Count > 0 Then
                ...
                End If
                Me._ddlReportTitles.SelectedIndex = 1
                End If
                    ...
                    With Me.AvailableReportParameters
                        ...
                        ElseIf _rblTaskStatus.SelectedValue = "Open" Or ((Request.Form(Me._rblTaskStatus.UniqueID) = "Open")) Then
                            If .EstimatedDueDate OrElse .DueDate Then
                                ...
                                End If
                                _dtDueDate.ShowReportDateRange = (_rblTaskStatus.SelectedValue = "Open")
                        ...
                        If RefSite.ToUpper = "IRIS" Then
                            ...
                            ph.Controls.Add(_ddlSourceSystem)
                            ph.Controls.Add(New LiteralControl("&nbsp;&nbsp;"))
                            ph.Controls.Add(_txtIncidentEventNumber)
                            If .SourceSystem Then
                                AddControlsToTable(ph, _lblIncidentEventNumber, 2)
                            End If
                        Else
                            Me._CreatedBy.Visible = .CreatedBy
                            If .CreatedBy Then
                            ...
                            If .Role Then
                                ...
                                '_ResponsiblePerson.EnableViewState = False
                                AddControlsToTable(_RoleList, Nothing, 2, False)
                            End If
                            ...
                            ph.Controls.Add(_txtIncidentEventNumber)
                            If .SourceSystem Then
                                AddControlsToTable(ph, _lblIncidentEventNumber, 1)
                            End If
                        End If
        ...
        If Me.SearchMode = TaskSearchMode.View Then
            ...
            If taskSearch IsNot Nothing Then
                With taskSearch
                    ...
                    Else
                        If .Responsible.Length > 0 Then Me._ResponsiblePerson.PopulateEmployeeList(.Responsible)
                        'Me._ResponsiblePerson.DefaultUserName = .Responsible
                    End If
                    'End If
        ...
        Try
            ...
            If newIncident IsNot Nothing Then
                ...
                If Me.SearchMode = TaskSearchMode.Reporting Then
                    ...
                    If AvailableReportParameters.GMSElement = True Then
                        ' Me._cblIncidentType.Items.Clear()
                        '        _cblActivity.Items.Clear()
                        '        _cblIncidentType.Text = ""
...
#Region "Events"
    ...
    Private Sub HandlePageLoad()
        ...
        If Me.SearchMode = TaskSearchMode.View Then
            ...
            If Not Page.IsPostBack Then
                Dim taskHeaderNumber As String = String.Empty
                'User Task Header Number from query string if available else use defaults.
                If Request.QueryString("HeaderNumber") Is Nothing OrElse IsNumeric(Request.QueryString("HeaderNumber")) = False Then
                    taskHeaderNumber = String.Empty
                Else
                    taskHeaderNumber = Request.QueryString("HeaderNumber")
                End If

                Dim taskNumber As String = String.Empty
                If Request.QueryString("TaskNumber") IsNot Nothing AndAlso IsNumeric(Request.QueryString("TaskNumber")) Then
                    Session.Item("TaskNumber") = Request.QueryString("TaskNumber")
                End If

                If taskHeaderNumber.Length > 0 Then
                    Session.Item("HeaderNumber") = taskHeaderNumber
                    Session.Remove("TaskSearchCriteria")

                    IP.Bids.SharedFunctions.ResponseRedirect(Request.Url.AbsolutePath)
                    Exit Sub
                ElseIf Session.Item("HeaderNumber") IsNot Nothing Then
                    taskHeaderNumber = Session.Item("HeaderNumber")
                End If
                PopulateData()
                ...
                Me._dtClosedDate.StartDate = ""
                Me._dtClosedDate.EndDate = ""
                Me._dtDueDate.EndDate = "" 'Now.AddMonths(1).ToShortDateString
                Me._dtDueDate.StartDate = "" 'New Date(Now.Year(), 1, 1).ToShortDateString
                'Me._dtDueDate.DateRange = User_Controls_JQDateRange.JQDateRange.YearToDate
                Me._dtHeaderDate.StartDate = ""
                Me._dtHeaderDate.EndDate = ""
                _ddlSearchOptions.Visible = True
                Dim defaults As RIUserDefaults.CurrentUserDefaults = Nothing '  Specialized.StringDictionary = Nothing
                If IP.Bids.SharedFunctions.GetCurrentUser IsNot Nothing AndAlso IP.Bids.SharedFunctions.GetCurrentUser.UserDefaults IsNot Nothing Then
                    defaults = IP.Bids.SharedFunctions.GetCurrentUser.UserDefaults
                End If
                If defaults IsNot Nothing Then
                    ...
                    Me._CreatedBy.PlantCode = defaults.GetDefaultValue(RIUserDefaults.UserProfileTypes.PlantCode, Me._ReportingSite.FacilityValue)
                End If
                'If defaults.ContainsKey("Facility") Then
                '    Me._ResponsiblePerson.PlantCode = defaults.Item("Facility")
                '    Me._CreatedBy.PlantCode = defaults.Item("Facility")
                'Else
                '    Me._ResponsiblePerson.PlantCode = Me._ReportingSite.FacilityValue
                '    Me._CreatedBy.PlantCode = Me._ReportingSite.FacilityValue
                'End If
                'If defaults.DoesDefaultValueExist(RIUserDefaults.UserProfileTypes.TaskSearchView) Then '("TaskSearchView") Then
                '    If _ddlSearchOptions.Items.FindByValue(defaults.GetDefaultValue(RIUserDefaults.UserProfileTypes.TaskSearchView)) IsNot Nothing Then
                '        _ddlSearchOptions.ClearSelection()
                '        _ddlSearchOptions.Items.FindByValue(defaults.GetDefaultValue(RIUserDefaults.UserProfileTypes.TaskSearchView)).Selected = True
                '    End If
                'Else
                '    If _ddlSearchOptions.Items.FindByValue("Month") IsNot Nothing Then
                '        _ddlSearchOptions.ClearSelection()
                '        _ddlSearchOptions.Items.FindByValue("Month").Selected = True
                '    End If
                'End If
                If defaults.DoesDefaultValueExist(RIUserDefaults.UserProfileTypes.TaskSearchStatus) Then
                    If Me._cblTaskStatus.Items.FindByValue(defaults.GetDefaultValue(RIUserDefaults.UserProfileTypes.TaskSearchStatus)) IsNot Nothing Then
                        Me._cblTaskStatus.ClearSelection()
                        Me._cblTaskStatus.Items.FindByValue(defaults.GetDefaultValue(RIUserDefaults.UserProfileTypes.TaskSearchStatus)).Selected = True
                    End If
                Else
                    If Me._cblTaskStatus.Items.FindByValue("1|-1") IsNot Nothing Then
                        Me._cblTaskStatus.ClearSelection()
                        Me._cblTaskStatus.Items.FindByValue("1|-1").Selected = True
                    End If
                End If

                Session.Remove("HeaderNumber")
                If taskHeaderNumber.Length = 0 Then
                    If defaults.DoesDefaultValueExist(RIUserDefaults.UserProfileTypes.PlantCode) Then
                        Me._ResponsiblePerson.PlantCode = defaults.GetDefaultValue(RIUserDefaults.UserProfileTypes.PlantCode)
                    End If
                    'Display the current users calendar
                    If performSearch = False Then
                        Me._ResponsiblePerson.DefaultUserName = IP.Bids.SharedFunctions.GetCurrentUser.Username
                        Me._ResponsiblePerson.PopulateEmployeeList(IP.Bids.SharedFunctions.GetCurrentUser.Username)
                    End If

                    'If Me._ResponsiblePerson.SelectedValue.Length > 0 And Me._ResponsiblePerson.DefaultUserName.Length > 0 Then
                    '    Me._ResponsiblePerson.SelectedValue = Me._ResponsiblePerson.DefaultUserName ' & Space(500) & "|" & Me._ResponsiblePerson.DefaultUserName
                    '    'Else
                    '    'Me._txtResponsiblePerson.Text = String.Empty
                    'End If
                Else
                        Me._txtHeaderNumber.Text = taskHeaderNumber
                    currentTab = 2
                    ...
                    End If
                End If
                'If _ddlSearchOptions.SelectedItem Is Nothing Then
                '    _ddlSearchOptions.SelectedIndex = 2
                'End If
                ReselectControls()
                If taskHeaderNumber.Length > 0 Then Me.DisplaySearchResults()
                If performSearch Then DisplaySearchResults()
            Else
                'If Me._dtDueDate.StartDate.Length = 0 Then
                '    Me._dtDueDate.DateRange = UserControlsJQDateRange.JQDateRange.CurrentYear
                'End If
            ...
            Else
                For j As Integer = 1 To worksheet.Columns.Count
                    ...
                    ElseIf j = HeaderTitleCol OrElse j = HeaderDescCol OrElse j = ItemDescCol OrElse j = ItemTitleCol Then
                        'worksheet.Row(i).Cell(j).SetValue(Translation.TranslateUsingApi(worksheet.Row(i).Cell(j).Value))
    ...
    Private Sub DisplayReport(ByVal taskSearch As TaskListingBll)
        ...
        Dim url As String = String.Format("http://gpimv.graphicpkg.com/CEReporting/CrystalReportDisplay.aspx" & mttReportQry)
    ...
    Private Sub DisplayHeaderList(ByVal listing As System.Collections.Generic.List(Of TaskListing))
        'Dim listing As System.Collections.Generic.List(Of TaskListing)

        If listing IsNot Nothing Then

            Dim sortDir As SortDirection = SortDirection.Ascending
            'listing = taskSearch.GetHeaderItemListing 'taskSearch.GetHeaderListing()

            'Dim query1 = From process As TaskListing In listing _
            '            Group process By process.TaskHeaderSeqId
            '            Select TaskHeaderSeqId, ProcCount _
            '            Order By ProcCount Descending, TaskHeaderSeqId


            Dim query = (From process As TaskListing In listing
                         Select New TaskListing(process.ActivityName, process.ActivitySeqID, 0, process.BusinessUnitArea, "", "", "", "", "", "", process.HeaderCreateDate, process.HeaderCreatedBy, process.HeaderDesc, "", "", process.HeaderTitle, "", "", "", "", process.Line, process.PlantCode, "", "", process.SiteName, "", "", "", process.TaskHeaderSeqId, 0, process.TaskType, "", "", "", "", "", "", "", "", 1, "", "", 0, 0)).Distinct(New TaskComparer)




            If query IsNot Nothing AndAlso query.Count > 0 Then
                _gvCalendarListing.DataSource = query    'listing.Take(500)
                _gvCalendarListing.DataBind()
                _gvCalendarListing.UseAccessibleHeader = True
                _gvCalendarListing.HeaderRow.TableSection = TableRowSection.TableHeader
                ScriptManager.RegisterStartupScript(Me._udpCalendarView, _udpCalendarView.GetType(), "SortGrid", "$(function(){{$('#" & _gvCalendarListing.ClientID & "').tablesorter({headers:{0:{sorter:'dd mm yyyy'},debug:true}}); }});", True)
                _tabSearchResults.Visible = True
                _tabSearchTabs.visible = True
                'Me._tabTaskHeaderListing.Visible = True
            Else
                HandleNoResultsFound()
                ' Me._tabTaskHeaderListing.Visible = False
            End If
        End If
    End Sub
    ...
    Private Sub DisplaySearchResults(ByVal Optional exportTasks As Boolean = False)
        Dim taskSearch As TaskListingBll
        ...
        If listing Is Nothing Then
            If IP.Bids.SharedFunctions.GetPageError.Length > 0 Then
                DisplayMessage(IP.Bids.SharedFunctions.GetPageError)
            End If
        End If
        DisplayTaskList(listing)
        DisplayCalendarView(listing)
        DisplayHeaderList(listing)
        If exportTasks Then ExportToExcel(taskSearch)
    ...
    Protected Sub _btnExportTasks_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles _btnExportTasks.Click
        DisplaySearchResults(exportTasks:=True)
        'Dim taskSearch As TaskListingBll
...
#End Region
    ...
    Private Sub UserControlsTaskSearch_Init(sender As Object, e As EventArgs) Handles Me.Init
        'If SearchMode = TaskSearchMode.Reporting Then
        '    DisplaySearchSelections()
        'End If
...
Public Class TaskComparer
    ...
    Public Function TaskComparer_Equals(ByVal x As IP.MEAS.BO.TaskListing, ByVal y As IP.MEAS.BO.TaskListing) As Boolean Implements System.Collections.Generic.IEqualityComparer(Of IP.MEAS.BO.TaskListing).Equals
        ' Reference same
        ...
        Return x.TaskHeaderSeqId = y.TaskHeaderSeqId
    End Function

    Public Function TaskComparer_GetHashCode(ByVal obj As IP.MEAS.BO.TaskListing) As Integer Implements System.Collections.Generic.IEqualityComparer(Of IP.MEAS.BO.TaskListing).GetHashCode
        Return obj.TaskHeaderSeqId.GetHashCode()
    End Function
End Class