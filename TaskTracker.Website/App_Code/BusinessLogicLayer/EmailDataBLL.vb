...
Public NotInheritable Class EmailDataBll
    ...
    Public Shared Function GetAndSendImmediateEmail(ByVal taskItemNumber As Integer, ByVal taskHeaderNumber As Integer) As Boolean
        ...
        Try
            ...
            Dim generaltasktrackerbll As New GeneralTaskTrackerBll

            Dim myrecords As DataSet = GeneralTaskTrackerBll.GetTaskItem(taskItemNumber)

            If myrecords IsNot Nothing Then
                If myrecords.Tables.Count = 1 Then
                    dr = myrecords.Tables(0).CreateDataReader
                    ...
                    While dr.Read()
                        ...
                        strEmailAddress = dr.Item("email")

                        'strRecType = "IS THIS ENTERED - WHAT IS RECTYPE"   'IP.Bids.SharedFunctions.LocalizeValue(rowItem.RECTYPE, True)
                        strRecType = ""
                        ...
                        strDueDate = dr.Item("DUEDATE")
                        strTitle = dr.Item("TITLE")

                        If dr.Item("RESPONSIBLENAME").Trim.Length = 0 Then
                            strResponsible = ""
                        Else
                            strResponsible = dr.Item("RESPONSIBLENAME")
                        End If

                        'strBusUnitMgr = rowItem.MGR
                        strTaskID = dr.Item("TASKITEMSEQID")
                        strCreatedBy = dr.Item("CREATEDBY")
                        strHeaderTitle = taskHeader.CurrentTaskHeaderRecord.Title
                        strDescription = dr.Item("DESCRIPTION")

                        strActivity = "" 'this is    '"WHAT IS THIS - strActivity?"
                        ...
                        ' strComments = dr.Item("COMMENTS")
                        strComments = ""

                        strTaskHeaderID = dr.Item("TASKHEADERSEQID")
                        strTaskItemID = dr.Item("TASKITEMSEQID")
                        ...
                        sbEmailBody.Append("{0}" & strHeaderTitle)
    ...
    Public Shared Function GetAndSendImmediateEmailOLD(ByVal taskItemNumber As Integer, ByVal taskHeaderNumber As Integer) As Boolean
        Dim adapter As TaskDetailTableAdapters.ImmediateEmailTableAdapter
        Dim table As TaskDetail.ImmediateEmailDataTable = Nothing
        Dim emptyCursor As Object = Nothing
        Dim retVal As Boolean
        ...
        Try
            ...
            'DataClean(dr.Item("USERNAME")).ToUpper
    ...
    '        For i As Integer = 0 To myrecords.Tables(0).Rows.Count - 1
    '            Dim someVar As Integer = Integer.Parse(myrecords.Tables(0).Rows(i)(0).ToString())
    '        Next
    ...
    '            'jeb need to fix
    '            strRecType = "Entered"   'IP.Bids.SharedFunctions.LocalizeValue(rowItem.RECTYPE, True)
    ...
    '        strActivity = "WHAT IS THIS - strActivity?"
